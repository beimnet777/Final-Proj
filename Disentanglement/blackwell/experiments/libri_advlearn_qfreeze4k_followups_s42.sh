#!/usr/bin/env bash
# Follow-up learned-routing quota-freeze experiments.
#
# Goal: keep the learned hard routing story alive without repeating the already
# completed 4k/5k/6k quota sweep.  These all freeze learned routing at 4k and
# keep the learned route-local TopK quota measured at freeze time.  The three
# branches test what might clean z_L speaker leakage after the route target
# becomes stationary:
#
#   1) stronger post-freeze speaker GRL norm, same 12k total budget
#   2) reset adversary heads at freeze, no stronger GRL, 16k total budget
#   3) longer post-freeze continuation only, no reset/no stronger GRL, 16k total
#
...
STRONG_POST_GRL_TARGET="${STRONG_POST_GRL_TARGET:-0.0003}"
...
# This is a checkpoint-head sanity check, not a trained probe.  Keep it off by
# default so the search suite is strictly linear PR/SID probes unless requested.
...
total_tag() {
    ...
    if (( total_steps % 1000 == 0 )); then
        printf '%sk' "$((total_steps / 1000))"
    else
        printf '%s' "$total_steps"
    fi
}
...
run_base_to_freeze() {
    ...
    if [[ "$SKIP_TRAINING" == "1" ]]; then
        echo "ERROR: SKIP_TRAINING=1 but usable step-${FREEZE_STEP} base freeze checkpoint is unavailable: $route_ckpt" >&2
        return 3
    fi
    ...
    local base_command=(
        ...
        $(learned_train_common_args "$base_run_name" "$total_steps" "$BASE_GRL_TARGET")
...
run_branch_from_freeze() {
    ...
    if [[ "$SKIP_TRAINING" == "0" ]]; then
        ...
        RUN_DESCRIPTION="Branch: freeze learned routing at ${FREEZE_STEP}, enforce learned route-local TopK quota, total=${total_steps}, post_gn=${post_grl_target}, reset_adv=${reset_adversaries}"
        local continue_command=(
            python -u Disentanglement/run.py
            $(learned_train_common_args "$branch_run_name" "$total_steps" "$post_grl_target")
            --resume "$route_ckpt"
        ...
        if [[ "$reset_adversaries" == "1" ]]; then
            continue_command+=(--reset_adversaries_on_resume)
        fi
...
echo "Learned-routing 4k quota-freeze follow-up suite"
...
base12_run="libri_advlearn_hardqfreeze${FREEZE_STEP}_base_dann${DANN_RAMP_STEPS}_gn00015_gp02_aux64_12k_s${SEED}"
base16_run="libri_advlearn_hardqfreeze${FREEZE_STEP}_base_dann${DANN_RAMP_STEPS}_gn00015_gp02_aux64_16k_s${SEED}"
base12_ckpt="$BLACKWELL_OUTPUT_ROOT/$base12_run/checkpoints/latest-resume.pt"
base16_ckpt="$BLACKWELL_OUTPUT_ROOT/$base16_run/checkpoints/latest-resume.pt"

run_base_to_freeze "$base12_run" 12000
run_branch_from_freeze \
    "libri_advlearn_hardqfreeze${FREEZE_STEP}_postgn0003_dann${DANN_RAMP_STEPS}_gp02_aux64_12k_s${SEED}" \
    12000 \
    "$base12_ckpt" \
    "$STRONG_POST_GRL_TARGET" \
    0

run_base_to_freeze "$base16_run" 16000
run_branch_from_freeze \
    "libri_advlearn_hardqfreeze${FREEZE_STEP}_resetadv_dann${DANN_RAMP_STEPS}_gn00015_gp02_aux64_16k_s${SEED}" \
    16000 \
    "$base16_ckpt" \
    "$BASE_GRL_TARGET" \
    1
run_branch_from_freeze \
    "libri_advlearn_hardqfreeze${FREEZE_STEP}_long_dann${DANN_RAMP_STEPS}_gn00015_gp02_aux64_16k_s${SEED}" \
    16000 \
    "$base16_ckpt" \
    "$BASE_GRL_TARGET" \
    0
...
echo
echo "All learned-routing 4k quota-freeze follow-up jobs completed."
