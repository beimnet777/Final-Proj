...
# Controlled replacement for the cancelled two-branch robust-GRL run.
# z_L has one speaker-adversary branch only:
#   Linear projector -> GELU -> masked mean+std -> speaker classifier.
...
RUN_NAME="lr_gelu_statsgrl_gp015_hard_gn025_10k_seed${TRAIN_SEED}"
...
echo "=== Hard learned-routing single GELU stats-GRL, reduced norm, 10k ==="
echo "started          : $(date)"
echo "node             : ${SLURMD_NODENAME:-unknown}"
echo "gpu              : $(nvidia-smi --query-gpu=name --format=csv,noheader)"
echo "run_name         : ${RUN_NAME}"
echo "routing          : hard ST-Gumbel, binary L/P, no z_U"
echo "grl_head         : ONE branch: linear projector -> GELU -> mean+std -> classifier"
