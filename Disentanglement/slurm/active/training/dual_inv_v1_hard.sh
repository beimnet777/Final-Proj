...
#SBATCH --job-name=dual_inv_v1_hard
...
# v1 dual-invariance, HARD learned routing (n_routes=2, Gumbel-softmax STE,
# tau anneals 1.0 -> 0.1 over the full run).
...
RUN_NAME="dual_inv_v1_hard"
CKPT_DIR="${DIS_DIR}/checkpoints/${RUN_NAME}"
STAGE2_CKPT="${CKPT_DIR}/stage2_best.pt"

echo "=== ${RUN_NAME}: dual-invariance v1, HARD learned routing (n_routes=2) ==="
echo "started: $(date)  gpu: $(nvidia-smi --query-gpu=name --format=csv,noheader)"
echo "routing : HARD Gumbel-softmax STE, tau 1.0 -> 0.1 over 12k steps"
...
echo
echo "----- held-out diagnostic probe -----"
date
