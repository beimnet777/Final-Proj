...
#SBATCH --job-name=statsgrl_gp02
#SBATCH --output=/rds/user/bbg25/hpc-work/Thesis/Final-Proj/Disentanglement/logs/train/stage2/statsgrl_gp02/%x_%j.out
#SBATCH --error=/rds/user/bbg25/hpc-work/Thesis/Final-Proj/Disentanglement/logs/train/stage2/statsgrl_gp02/%x_%j.err

# Job2 dynamics fix:
#   - keep the successful fixed-block Job2 base;
#   - replace z_L speaker adversary with diagnostic-style stats pooling
#     (Linear -> ReLU -> masked mean+std -> SID), so training fights the
#     same leakage family that the strong post-hoc SID probe can detect;
#   - keep existing global gradient clipping explicit (max_norm=1.0);
#   - weaken z_P phoneme GRL to 0.2 to reduce late grl_p domination observed in
#     balance_diag while still discouraging linguistic leakage into z_P.
...
mkdir -p "${DIS_DIR}/logs/train/stage2/statsgrl_gp02"
cd "${DIS_DIR}"
...
echo "=== ${RUN_NAME} (${LABEL}): Job2 + stats-pool z_L GRL + grad clip + weaker grl_p ==="
echo "started       : $(date)"
...
${PYTHON} -u diag_probe/run.py \
    ...
    --run_name "diag_${RUN_NAME}_chas_way" \
