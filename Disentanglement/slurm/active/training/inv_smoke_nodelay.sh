...
# SMOKE (NO RAMP DELAY): full invariance weight from step 1 (inv_ramp_end=0).
#   Tests whether z_L needs to "form content first" or invariance can apply at once.
#   Watch:  inv -> should DROP from ~1.0;  recon stays sane;  pr low;  grl_p healthy.
#   grl_weight=0 = passive monitor.
...
echo "=== inv SMOKE (no delay): inv_weight=4.0 ramp_end=0 700 steps ===  $(date)"
...
${PYTHON} -u run.py \
    ...
    --checkpoint_dir "${DIS_DIR}/checkpoints/inv_smoke_nodelay" --runs_dir "${DIS_DIR}/runs/inv_smoke_nodelay" \
    --log_dir "${DIS_DIR}/logs" --seed 42

echo "Finished SMOKE (no delay): $(date)"
