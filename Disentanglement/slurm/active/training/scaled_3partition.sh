...
#SBATCH --job-name=scaled_3part
#SBATCH --array=0-2%3
#SBATCH --output=/rds/user/bbg25/hpc-work/Thesis/Final-Proj/Disentanglement/logs/train/stage2/scaled_3partition/%x_%A_%a.out
#SBATCH --error=/rds/user/bbg25/hpc-work/Thesis/Final-Proj/Disentanglement/logs/train/stage2/scaled_3partition/%x_%A_%a.err

# Three scaled UNIFIED runs (recon + PR + SID + 4 adversaries + AuxK, from scratch),
# identical except the PARTITION mechanism:
#   0  preact_fixed   fixed L/P/U blocks (equal) + GLOBAL top-k  → membership fixed, allocation emerges
#   1  routed_hard    learned routing, HARD Gumbel + entropy-min (spec)  → DL learns a decisive partition
#   2  routed_soft    learned routing, SOFT Gumbel (softmax)
#
# Shared: K=16384, top-64, AuxK (gm-bias + renorm), per-frame z_P recon (SID pools),
# z_U adversaries (anti-speaker + anti-phoneme), grl/grl_p=0.7, grl_u=0.5, awakened
# (lr_disc 1e-3, n_disc 3), local train-clean-360 (921 spk), 26k steps (4 passes/utt).
...
mkdir -p "${DIS_DIR}/logs/train/stage2/scaled_3partition"
cd "${DIS_DIR}"
...
RUN_NAME="scaled_${NAME}"
CKPT_DIR="${DIS_DIR}/checkpoints/${RUN_NAME}"
STAGE2_CKPT="${CKPT_DIR}/stage2_best.pt"

echo "=== scaled experiment ${i}: ${NAME} ===  $(date)"
echo "GPU $(nvidia-smi --query-gpu=name --format=csv,noheader)"
echo "partition: ${MASK[*]}   rho=${RHO}"
