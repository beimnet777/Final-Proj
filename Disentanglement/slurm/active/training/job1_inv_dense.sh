...
# JOB 1: invariance (pitch+formant) ON z_L  +  DENSE per-frame speaker GRL on z_L.
# Invariance removes pitch/formant-correlated speaker; the dense adversary mops up
# the residual.  grl_p keeps z_P clean.  Probe z_L,z_P only.
...
echo "=== JOB1 invariance + dense GRL ===  $(date)"; nvidia-smi --query-gpu=name --format=csv,noheader
${PYTHON} -u run.py \
