...
# Strongclean++ MSP follow-up.
#
# Builds from the best balanced MSP run so far:
...
#   - calibrate route-local TopK quota after freeze
#   - speaker z_L GRL per-frame target: 8e-4
#   - emotion z_L GRL per-frame target: 4e-4
#   - stronger anti-phone pressure on z_P: grl_p=0.35
#
# Task 0: slower DANN ramp to 12k, like strongclean.
# Task 1: faster DANN ramp to 6.8k, to give full adversary more post-freeze time.
#
# Submit:
#   cd /rds/user/bbg25/hpc-work/Thesis/Final-Proj/Disentanglement
#   sbatch msp/slurm/sweep_msp_strongclean_norm_followups.sh
#
# Submit only one case:
#   sbatch --array=1 msp/slurm/sweep_msp_strongclean_norm_followups.sh
...
SPEAKER_GRL_NORM_TARGET=0.0008
EMOTION_GRL_NORM_TARGET=0.0004
...
case "${TASK_ID}" in
  0)
    RUN_NAME=msp_hardqfreeze4000_scplusplus_spkgn0008_emogn0004_grlp035_dann12000_s42
    DANN_RAMP_STEPS=12000
    ;;
  1)
    RUN_NAME=msp_hardqfreeze4000_scplusplus_spkgn0008_emogn0004_grlp035_dann6800_s42
...
echo "=== MSP strongclean++ normalized follow-up ==="
echo "started    : $(date)"
...
echo "grl_norm   : speaker=${SPEAKER_GRL_NORM_TARGET} emotion=${EMOTION_GRL_NORM_TARGET}"
...
COMMON_ARGS=(
  ...
  --grl_emotion_grad_norm --grl_emotion_grad_norm_target "${EMOTION_GRL_NORM_TARGET}"
