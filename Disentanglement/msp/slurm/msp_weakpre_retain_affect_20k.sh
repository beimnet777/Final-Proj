...
# MSP binary L/P qfreeze run:
#   - 20k total steps
#   - learn routing to 4k, then exact-resume with frozen learned routes
#   - route-local TopK quota after freeze
#   - slow DANN ramp = 12k
#   - pre-freeze: moderate/weak adversarial pressure so P can become useful
#   - post-freeze: stronger content/affect cleanup once routes are stable
#
# Submit on HPC:
#   cd /rds/user/bbg25/hpc-work/Thesis/Final-Proj/Disentanglement
#   sbatch msp/slurm/msp_weakpre_retain_affect_20k.sh
...
RUN_NAME=msp_hardqfreeze4000_weakpre_retainaff_gn0003_grlp010to025_advpe010to020_20k_s42
...
# Cooperative weights: stronger speaker/affect retention than earlier MSP runs.
RECON_WEIGHT=1.0
PR_WEIGHT=0.8
SID_WEIGHT=1.0
PROSODY_WEIGHT=0.6
...
# Speaker cleanup: slightly gentler than the previous strong-clean 4e-4.
SPEAKER_GRL_WEIGHT=1.0
SPEAKER_GRL_NORM_TARGET=0.0003

# Pre-freeze adversaries: keep them present, but avoid early P starvation.
PRE_PHONEME_GRL_WEIGHT=0.10
PRE_PROSODY_GRL_WEIGHT=0.10
PRE_EMOTION_GRL_WEIGHT=0.10

# Post-freeze adversaries: cleanup after route/quota target is stable.
POST_PHONEME_GRL_WEIGHT=0.25
POST_PROSODY_GRL_WEIGHT=0.20
POST_EMOTION_GRL_WEIGHT=0.20
...
echo "=== MSP weak-pre / retain-affect qfreeze 20k ==="
echo "started    : $(date)"
...
echo "[phase 2] freeze learned routes + quota TopK, then continue with stronger post-freeze cleanup"
"${PYTHON}" -u -m msp.run "${BASE_ARGS[@]}" \
